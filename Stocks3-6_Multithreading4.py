...
print("Program start Time = ", totalStart)

date, bid, ask = np.loadtxt('GBPUSD1m.txt', unpack=True,
                            delimiter=',',
                            converters={0: mdates.bytespdate2num('%Y%m%d%H%M%S')})
print("csv import time = ", time.time() - totalStart)
def doPatternStorage(q):
    while True:
        patternArgs = q.get()
        patternStorage(patternArgs[0], patternArgs[1])
        q.task_done()
...
def patternStorage(startY, endY):
    ...
    len(performanceAr)
    print
    'Pattern storing took:', endTime - startTime
...
# tracking for breaking out of the while loop 
loopNum = 0

# declare just the queue
q = Queue()

# declare all threads (done outside the while loop so they can be reused
#       and reduce overhead opperation
for threadNum in range(NUM_OF_THREADS):
    t = threading.Thread(target=doPatternStorage, args=(q,))
    t.setDaemon(True)
    t.start()
...
while toWhat < dataLength:
    ...
    #print(dataLength)
    print("Prediction #: ", loopNum)
    
    for threadNum in range(NUM_OF_THREADS ):
        startingY = int((len(avgLine)/NUM_OF_THREADS) * (threadNum + 1) - (len(avgLine)/NUM_OF_THREADS))
        endingY = int(len(avgLine)/NUM_OF_THREADS) * (threadNum + 1)
        yArgs = [startingY, endingY]
        q.put(yArgs)
    ...
    loopNum +=1
    if (loopNum == NUM_OF_LOOPS):
        break
totalEnd = time.time()
print("Start Time = ", totalStart)
print("End Time =   ", totalEnd)
print("Runtime =    ", totalEnd - totalStart)
print("version 3")
